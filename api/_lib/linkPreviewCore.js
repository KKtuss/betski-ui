import { proxiedThumbnailUrl } from "./thumbnailProxy.js";
...
function extractTikTokStats(struct, html) {
  ...
  if (raw) {
    ...
    if (playCount2 != null || diggCount2 != null || shareCount2 != null) {
      return { playCount: playCount2, diggCount: diggCount2, shareCount: shareCount2 };
    }
  }
...
async function resolveLinkPreview(targetUrl) {
  ...
  if (lower.includes("tiktok")) {
    ...
    const tiktokOembed = !parsed?.thumb.thumbnail_url ? await fetchOembed(`https://www.tiktok.com/oembed?url=${encodeURIComponent(canonicalUrl)}`) : null;
